...
class SeriesConverterTests {
    ...
    private val modelSeries = Series(
        id, title, description, Thumbnail(thumbnailPath, thumbnailExtension),
        SeriesInfo(available), startYear, endYear, rating
    )

    // Mock Series from DB
    private val dbSeries = com.jaime.marvelviewer.db.Series(
        id, title, description, "$thumbnailPath.$thumbnailExtension",
